...
import type { TokenHealthResult } from '@/shared/types';
...
export function Settings() {
  ...
  // Token health state
  const [healthResult, setHealthResult] = useState<TokenHealthResult | null>(null);
  const [healthScanning, setHealthScanning] = useState(false);
  const [healthPurging, setHealthPurging] = useState(false);
  const [healthPurged, setHealthPurged] = useState<number | null>(null);
  ...
  const handlePurge = async () => {
    if (!confirm('Remove all invalid tokens? You will need to ask the sender to re-send them.')) return;
    setHealthPurging(true);
    try {
      const { purged } = await purgeInvalidTokens();
      setHealthPurged(purged);
      setHealthResult(null);
    } catch (error) {
      console.error('Purge failed:', error);
    } finally {
      setHealthPurging(false);
    }
  };
  ...
  return (
    <div className="p-4">
      ...
      {/* Token Health */}
      <div className="mb-6">
        <h3 className="text-sm text-gray-400 mb-3">Token Health</h3>

        {healthPurged !== null && (
          <div className="bg-green-900/20 border border-green-700/50 rounded-lg p-3 mb-3">
            <p className="text-sm text-green-400">
              Purged {healthPurged} invalid token{healthPurged !== 1 ? 's' : ''}. Balances refreshed.
            </p>
          </div>
        )}
        ...
        {healthResult ? (
          <div className="space-y-3">
            {healthResult.invalid.length === 0 ? (
              <div className="bg-green-900/20 border border-green-700/50 rounded-lg p-3">
                <p className="text-sm text-green-400">
                  All {healthResult.total} tokens are healthy.
                </p>
              </div>
            ) : (
              <>
                <div className="bg-yellow-900/20 border border-yellow-700/50 rounded-lg p-3">
                  <p className="text-sm text-yellow-400 mb-2">
                    {healthResult.invalid.length} of {healthResult.total} tokens are invalid.
                  </p>
                  <p className="text-xs text-yellow-400/70">
                    These tokens were received before the SDK upgrade and cannot be spent.
                    Their ownership was never transferred on-chain.
                    Purge them and ask the sender to re-send.
                  </p>
                </div>

                <div className="bg-gray-800 rounded-lg p-3 space-y-2 max-h-40 overflow-y-auto">
                  {healthResult.invalid.map((tok) => (
                    <div key={tok.id} className="flex justify-between items-center text-xs">
                      <span className="text-gray-300">{tok.symbol}</span>
                      <span className="text-gray-400 font-mono">{tok.amount}</span>
                    </div>
                  ))}
                </div>
                ...
                <button
                  onClick={handlePurge}
                  disabled={healthPurging}
                  className="w-full bg-red-600/20 hover:bg-red-600/30 text-red-400
                             py-2 px-4 rounded-lg text-sm transition-colors"
                >
                  {healthPurging
                    ? 'Purging...'
                    : `Purge ${healthResult.invalid.length} Invalid Token${healthResult.invalid.length !== 1 ? 's' : ''}`}
                </button>
              </>
            )}

            <button
              onClick={() => { setHealthResult(null); setHealthPurged(null); }}
              className="w-full bg-gray-700 hover:bg-gray-600 text-white py-2 px-4 rounded-lg
                         text-sm transition-colors"
            >
              Dismiss
          ...
          <button
            ...
            className="w-full bg-gray-800 hover:bg-gray-700 rounded-lg p-3 text-sm
                       text-gray-400 transition-colors"
          >
            {healthScanning ? 'Scanning...' : 'Check Token Health'}
          </button>
        )}
      </div>
